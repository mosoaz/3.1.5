...
function rolesUser(event) {
    ...
    if (roles.includes('2')) {
        rolesAdmin["id"] = 2;
        rolesAdmin["rolename"] = "USER";
        allRoles.push(rolesAdmin);
    }
    if (roles.includes('1')) {
        rolesUser["id"] = 1;
        rolesUser["rolename"] = "ADMIN";
...
function showDeleteModal(id) {
    ...
    fetch(request).then(res => res.json()).then(deleteUser => {
            ...
            document.getElementById('lastNameDel').setAttribute('value', deleteUser.surname);
            ...
            if (getRoles(deleteUser.roles).includes("ADMIN") && getRoles(deleteUser.roles).includes("USER")) {
...
function showEditModal(id) {
    ...
    fetch(request).then(res => res.json()).then(editUser => {
            ...
            document.getElementById('lastNameRed').setAttribute('value', editUser.surname);
